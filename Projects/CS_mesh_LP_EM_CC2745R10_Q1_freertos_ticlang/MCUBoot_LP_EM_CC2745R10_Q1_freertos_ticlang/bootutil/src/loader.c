...
context_boot_go(struct boot_loader_state *state, struct boot_rsp *rsp)
{
    ...
    if (img_cnt) {
        ...
        {
            BOOT_LOG_INF("There are %d images", img_cnt);

            slot = 0;

            int flag_var = HWREG(PMCTL_BASE + PMCTL_O_AONRSTA1); //set flag_var to the value of the register to determine image swap
            BOOT_LOG_INF("Value of flag_var : %x", flag_var);
            BOOT_LOG_INF("Trimmed value of flag_var : %x", (flag_var & 0x0003FFFF));
            if((flag_var & 0x00000001) == 0x1)
            {
                slot = 1;
            }
            BOOT_LOG_INF("Slot : %x", slot);
