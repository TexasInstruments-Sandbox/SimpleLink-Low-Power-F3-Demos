...
void Connection_eventEventHandler(uint32 event, BLEAppUtil_msgHdr_t *pMsgData);
...
BLEAppUtil_EventHandler_t connectionEventHandler =
{
    .handlerType    = BLEAPPUTIL_CONN_NOTI_TYPE,
    .pEventHandler  = Connection_eventEventHandler,
...
//    status = BLEAppUtil_registerEventHandler(&connectionEventHandler);
//    if(status != SUCCESS)
//    {
//        return(status);
//    }
